...
func TestApply(t *testing.T) {
	for _, tc := range difftest.TestCases {
		t.Run(tc.Name, func(t *testing.T) {
			got, err := gotextdiff.Apply(tc.In, tc.Edits)
			if err != nil {
			...
			if tc.LineEdits != nil {
				got, err := gotextdiff.Apply(tc.In, tc.LineEdits)
				if err != nil {
...
func TestNEdits(t *testing.T) {
	for _, tc := range difftest.TestCases {
		edits := gotextdiff.Strings(tc.In, tc.Out)
		got, err := gotextdiff.Apply(tc.In, edits)
		if err != nil {
...
func TestNRandom(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		a := randstr("abω", 16)
		b := randstr("abωc", 16)
		edits := gotextdiff.Strings(a, b)
		got, err := gotextdiff.Apply(a, edits)
		if err != nil {
			t.Fatalf("Apply failed: %v", err)
		}
		if got != b {
...
func TestLineEdits(t *testing.T) {
	for _, tc := range difftest.TestCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			got, err := gotextdiff.LineEdits(tc.In, tc.Edits)
			if err != nil {
			...
			fixed, err := gotextdiff.Apply(tc.In, got)
			if err != nil {
				t.Error(err)
			}
			if fixed != tc.Out {
...
func TestToUnified(t *testing.T) {
	...
	for _, tc := range difftest.TestCases {
		t.Run(tc.Name, func(t *testing.T) {
			unified, err := gotextdiff.ToUnified(tc.In, tc.Edits, gotextdiff.DefaultContextLines)
			if err != nil {
				t.Fatal(err)
			}
			if unified == "" {
...
func TestRegressionOld001(t *testing.T) {
	...
	diffs := gotextdiff.Strings(a, b)
	got, err := gotextdiff.Apply(a, diffs)
	if err != nil {
		t.Fatalf("Apply failed: %v", err)
	}
	if got != b {
